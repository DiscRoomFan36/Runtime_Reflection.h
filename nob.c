...
#define ARGUMENTS                                               \
    ...
    X(all,          "build all targets. [example, tests]")      \
    ...
    X(example,      "build example in debug mode")              \
    X(tests,        "build the test file")                      \
...
int main(int argc, char **argv) {
    ...
    #define TEST_MA_PATH "/home/fletcher/Programming/TEST_MA.h/TEST_MA.h"
    if (file_exists(TEST_MA_PATH) == 1) {
        bool result = copy_file(TEST_MA_PATH, THIRDPARTY_FOLDER"TEST_MA.h");
        if (!result) return 1;
    }


    // dont make a build folder if your not building anything.
    bool building_anything = flags.example || flags.tests;
    if (building_anything) {
        mkdir_if_not_exists(BUILD_FOLDER);
    }

    if (flags.example) {
        if (!build_example())       exit(EXIT_FAILURE);
    }
    if (flags.tests) {
        if (!build_tests())         exit(EXIT_FAILURE);
    }
...
bool build_example(void) {
    ...
    cmd_append(&cmd, "-o", BUILD_FOLDER"example");
...
bool build_tests(void) {
    ...
    cmd_append(&cmd, "-o", BUILD_FOLDER"tests");
    cmd_append(&cmd, "./tests/tests_main.c");
